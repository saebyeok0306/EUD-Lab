...
def get_cunit_nav_list():
    source_path = Path("docs/CUnit")
    file_list = []
    ...
    js_content = f"window.cunitList = {json.dumps(file_list, ensure_ascii=False)};"

    with mkdocs_gen_files.open("javascripts/cunit_list.js", "w", encoding="utf-8") as fs:
        fs.write(js_content)

    print("Generated cunit_list.js with file list for JavaScript.")

get_address_nav_list()
try:
    get_cunit_nav_list()
